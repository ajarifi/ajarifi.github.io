---
title: "Activities"
permalink: /activities/
author_profile: false
---

<style>
/* Collapsible */
details {
  margin-bottom: 1.5cm;
  border-bottom: 1px solid #ddd;
  padding-bottom: 10px;
}

summary {
  font-size: 1.6em;
  font-weight: bold;
  color: #000080;
  cursor: pointer;
}

summary:hover {
  color: #1a4fbf;
}

summary::-webkit-details-marker {
  display: none;
}

/* List spacing */
ol li {
  margin-bottom: 25px;
}
</style>

<p align="justify">
I enjoy being involved in organizing scientific events. 
Please feel free to reach out if you would like to involve me in organizing seminars, workshops, or related activities.
</p>

<p style="margin-bottom:1.5cm;"></p>

<!-- Upcoming -->
<details open>
  <summary>📅 Upcoming Activities (2026) </summary>

  <ol>

    <li>
      <b>2nd Southeast Asian Workshop on Nuclear and Hadron Physics</b><br> 
      <button class="btn--article">Workshop</button> <b>Link</b>: <a href="https://indico.global/event/18065/">[Indico page] </a> (Organizer) <br>
      <b>Date & time</b>: December 1-5, 2026<br>
      <b>Place</b>: Khon Kaen University, Thailand
    </li>
    
    <li>
      <b>International Workshop on Origin of Mass and Matter in the Universe</b><br> 
      <button class="btn--article">Workshop</button> <b>Link</b>: <a href="https://indico.global/event/16936/">[Indico page]</a> (Organizer)<br>
      <b>Date & time</b>: July 21-24, 2026<br>
      <b>Place</b>: APCTP, South Korea
    </li>
  
    <li>
      <b>Quarks and Compact Stars (QCS 2026)</b><br> 
      <button class="btn--article">Workshop</button> <b>Link</b>: <a href="https://indico2.riken.jp/event/5301/">[Indico page]</a> (Visa support)<br>
      <b>Date & time</b>: April 29 - May 02, 2026<br>
      <b>Place</b>: JAEA Tokai Mirai Base, Ibaraki, Japan
    </li>
    
  </ol>

</details>

<!-- Past -->
<details>
  <summary>📚 Past Activities</summary>

  <!-- 2025 -->
  <details>
    <summary>2025</summary>
    <ol>
       <li>
        <b>1st Southeast Asian Workshop on Nuclear and Hadron Physics</b><br> 
        <button class="btn--article">Workshop</button> <b>Link</b>: <a href="https://indico.global/event/13949/">[Indico page]</a> (co-chair)<br>
        <b>Date & time</b>: 19–21 August 2025<br>
        <b>Place</b>: UGM, Yogyakarta, Indonesia
      </li>
    </ol>
  </details>

  <!-- 2024 -->
  <details>
    <summary>2024</summary>
    <ol>
      <li>
        <b>International Workshop on Quark Structure of Hadrons</b><br> 
        <button class="btn--article">RIKEN Workshop</button> <b>Link</b>: <a href="https://indico2.riken.jp/event/4881/">[Indico page]</a> (chair)<br>
        <b>Date & time</b>: 9–10 August 2024<br>
        <b>Place</b>: RIKEN (hybrid)
      </li>
    </ol>
  </details>

  <!-- 2022 -->
  <details>
    <summary>2022</summary>
    <ol>
      <li>
        <b>Artificial-Neural Network and Polarized-Target Technology for Probing Nucleon Structure</b><br> 
        <button class="btn--article-blue">APCTP Seminar</button> 
        <a href="https://ajarifi.github.io/seminar/9akbar.html">[Abstract]</a> <br>
        <b>Speaker</b>: Zulkaida Akbar (University of Virginia, USA)<br>
        <b>Date & time</b>: 10:30 am KST, Oct 17, 2022<br>
        <b>Place</b>: Online (Zoom)
      </li>
    
      <li>
        <b>Hadron Structure in the Light-Front Dynamics</b><br> 
        <button class="btn--article-blue">APCTP Seminar</button> 
        <a href="https://ajarifi.github.io/seminar/8yongwoo.html">[Abstract]</a>
        <a href="/files/yongwoo.jpg">[Poster]</a><br>
        <b>Speaker</b>: Yongwoo Choi (Inha University, Korea)<br>
        <b>Date & time</b>: 11 am KST, Sept 21, 2022<br>
        <b>Place</b>: Online (Zoom)
      </li>
      <li>
        <b>Light-Front Wavefunctions of Mesons by Design</b><br> 
        <button class="btn--article-blue">APCTP Seminar</button> 
        <a href="https://ajarifi.github.io/seminar/7meijian.html">[Abstract]</a>
        <a href="/files/meijian.pdf">[Poster]</a><br>
        <b>Speaker</b>: Meijian Li (University of Santiago de Compostela, Spain)<br>
        <b>Date & time</b>: 4 pm KST, July 26, 2022<br>
        <b>Place</b>: Online (Zoom)
      </li>
      
      <li>
        <b>Singly Heavy Baryons from Chiral Symmetry</b><br> 
        <button class="btn--article-blue">APCTP Seminar</button>
        <a href="https://ajarifi.github.io/seminar/6suenaga.html">[Abstract]</a>
        <a href="/files/suenaga.jpg">[Poster]</a><br>
        <b>Speaker</b>: Daiki Suenaga (RIKEN, Japan)<br>
        <b>Date & time</b>: 2 pm KST, June 23, 2022<br>
        <b>Place</b>: Online (Zoom)
      </li>
    </ol>
  </details>

  
  <!-- 2021 -->
  <details>
    <summary>2021</summary>
    <ol>
    
      <li>
        <b>Resonance and Decay of the N(1875) in Triangle Singularity</b><br> 
        <button class="btn--article-blue">APCTP Seminar</button>
        <a href="https://ajarifi.github.io/seminar/5daris.html">[Abstract]</a>
        <a href="/files/samart.jpg">[Poster]</a><br>
        <b>Speaker</b>: Daris Samart (Khon Kaen University, Thailand)<br>
        <b>Date & time</b>: 3 pm KST, Oct 19, 2021<br>
        <b>Place</b>: Online (Zoom)
      </li>
    
    
      <li style="margin-bottom: 25px;"><b>Molecular picture of the Omega(2012)</b><br> 
          <button class="btn--article-blue"> APCTP Seminar </button>&nbsp; 
          <a href="https://ajarifi.github.io/seminar/4ikeno.html">[Abstract]</a> 
          <a href="/files/Ikeno.jpg">[Poster]</a> <br>
          <b>Speaker</b>: Natsumi Ikeno (Tottori U., Japan)<br>
          <b>Date & time</b>: 2 pm KST, Sept 28, 2021<br>
          <b>Place</b>: Online (Zoom)
      </li>
      
      <li style="margin-bottom: 25px;"><b>Time evolution of lepton family number; application to non-relativistic neutrinos</b><br> 
          <button class="btn--article-blue"> APCTP Seminar </button> <a href="https://ajarifi.github.io/seminar/3apriadi.html">[Abstract]</a> <a href="/files/apriadi.pdf">[Poster]</a>  &nbsp; <br>
          <b>Speaker</b>: Apriadi Salim Adam (The Indonesian Institute of Sciences (LIPI), Indonesia)<br>
          <b>Date & time</b>: 3 pm KST, Aug 25, 2021<br>
          <b>Place</b>: Online (Zoom) 
      </li>
      
      <li style="margin-bottom: 25px;"><b>Study of excited open-heavy flavor mesons using HQET</b><br> 
          <button class="btn--article-blue"> APCTP Seminar </button>&nbsp; 
          <a href="https://ajarifi.github.io/seminar/2keval.html">[Abstract]</a> <a href="/files/keval.pdf">[Poster]</a>  
          <br>
          <b>Speaker</b>: Keval M. Gandhi (Sardar Vallabhbhai National Inst. of Tech., India)<br>
          <b>Date & time</b>: 2 pm KST, June 29, 2021 <br>
          <b>Place</b>: Online (Zoom)
      </li>
    
      <li style="margin-bottom: 25px;"><b>Model-independent analysis of hadron-hadron scattering: a deep learning approach</b><br> 
          <button class="btn--article-blue"> APCTP Seminar </button> <a href="https://ajarifi.github.io/seminar/1denny">[Abstract]</a> <a href="/files/denny.pdf">[Poster]</a>  <br>
          <b>Speaker</b>: Denny Lane B. Sombillo (RCNP, Osaka U., Japan & NIP, U. of the Philippines)<br>
          <b>Date & time</b>: 11 am KST, May 27, 2021 <br>
          <b>Place</b>: Online (Zoom) 
      </li>
    </ol>
  </details>

</details>
