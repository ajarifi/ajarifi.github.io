---
title: "Research Grants"
permalink: /grants/
author_profile: false
---
  

<p align="justify">
My academic journey has been supported by scholarships and my research by various competitive and internal grants, 
motivating me to make meaningful and useful contributions to science and society.
</p>


<ol reversed>

  <b> 2026 </b>
  
  <li> <b>A. J. Arifi.</b> KAKENHI Early-Career Scientists. Japan Society for the Promotion of Science. <br>
    <button class="btn--article">Competitive grant</button> <span style="color:#900C3F">Principal Investigator</span>, Japan, 2026-2028 (3 years), 3.400.000 JPY
  </li>

  <li> D. Samart, et.al. APCTP Program Category 4: 2nd Southeast asean workshop on nuclear and hadron physics (SEA-NHP)<br>
    <button class="btn--article">Workshop grant</button> <span style="color:#900C3F">Co-Principal Investigator</span>, South Korea, 2026 (1 year), 4.000.000 KRW
  </li>

  <li> <b>A. J. Arifi</b>, et. al. APCTP Program Category 2: International workshop on origin of mass and matter in the universe<br>
    <button class="btn--article">Workshop grant</button>  <span style="color:#900C3F">Principal Investigator</span>, South Korea, 2026 (1 year), 5.000.000 KRW
  </li>

  <b> 2025 </b>
  
  <li> T. Mart, et. al. PUTI Q1, Universitas Indonesia<br>
    <button class="btn--article">Competitive grant</button> <span style="color:#900C3F">Project member</span>, Indonesia, 2025–2026 (1 year),  100.000.000 IDR
  </li>

  <li> <b>A. J. Arifi</b>, et. al. COREnet Program, Research Center for Nuclear Physics (RCNP)<br>
    <button class="btn--article">Competitive grant</button>  <span style="color:#900C3F">Principal Investigator</span>, Japan, 2025–2027 (2 years), 420,000 JPY.
  </li>

  <li><b>A. J. Arifi.</b> JAEA Postdoctoral Program <br>
    <button class="btn--article-black">Internal grant</button>  <span style="color:#900C3F">Principal Investigator</span>, Japan, 2025–2026 (1 year), 500,000 JPY
  </li>

  <b> 2024 </b>
  
  <li> <b>A. J. Arifi.</b> Additional SPDR Funds, RIKEN<br>
    <button class="btn--article">Competitive grant</button> <span style="color:#900C3F">Principal Investigator</span>, Japan, 2024–2025 (1 year), 500,000 JPY
  </li>

  <b> 2023 </b>
  <li> <b>A. J. Arifi.</b>  Special postdoctoral researcher (SPDR) program, RIKEN<br>
    <button class="btn--article-black">Internal grant</button>  <span style="color:#900C3F">Principal Investigator</span>, Japan,  2023–2026 (3 years), 1,000,000 JPY/year
  </li>

  <b> 2021 </b>
  <li> <b>A. J. Arifi.</b> Young Scientist Training (YST) Program, APCTP <br>
    <button class="btn--article-black">Internal grant</button> <span style="color:#900C3F">Principal Investigator</span>, South Korea, 2021–2023 (2 years), 5,000,000 KRW/year
  </li>

  <b> Scholarships </b>
  <li> <b>A. J. Arifi.</b> Japanese Government (MEXT) Scholarship, Doctoral program<br>
    <button class="btn--article-blue">Scholarship</button> Japan. 2017–2020 (3 years).
  </li>

  <li> <b>A. J. Arifi.</b> Japanese Government (MEXT) Scholarship, Master's program<br>
    <button class="btn--article-blue">Scholarship</button> Japan, 2015–2017 (2 years).
  </li>

  <li> <b>A. J. Arifi.</b> Tanoto Foundation Scholarship, Bachelor's program <br>
    <button class="btn--article-blue">Scholarship</button> Indonesia, 2012–2015 (3 years).
  </li>
</ol>
 
